...
class FloatingCameraSensor(Sensor):
    def __init__(self, env):
        ...
        self.env.gym.set_camera_location(self.rendering_camera, self.env.envs[0], labapi.Vec3(1.5, 1, 3.0),
                                        labapi.Vec3(0, 0, 0))
    ...
    def set_position(self, target_loc=None, cam_distance=None):
        ...
        if target_loc is None:
            ...
            target_loc = [bx, by, bz]
        self.env.gym.set_camera_location(self.rendering_camera, self.env.envs[0], labapi.Vec3(target_loc[0] + cam_distance[0],
                                                                                      target_loc[1] + cam_distance[1],
                                                                                      target_loc[2] + cam_distance[2]),
                                     labapi.Vec3(target_loc[0], target_loc[1], target_loc[2]))

    def get_observation(self, env_ids = None):
        self.env.gym.step_graphics(self.env.sim)
        self.env.gym.render_all_camera_sensors(self.env.sim)
        img = self.env.gym.get_camera_image(self.env.sim, self.env.envs[0], self.rendering_camera, labapi.IMAGE_COLOR)
        w, h = img.shape
